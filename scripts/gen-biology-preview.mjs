...
// Renders lesson bodies + quizzes through remark (markdown + tables) and links
// the same raster biology illustrations used by the learning application.
...
const md = (t) => unified()
  ...
  .toString()
  .replaceAll(
    'src="/content/biology-grade-12/',
    'src="../../apsara-elearning-web/public/content/biology-grade-12/',
  );
...
course.modules.forEach((m0) => {
  ...
  m0.lessons.forEach((l) => {
    ...
    body += `<article class="lesson" id="${l.slug}"><h3 class="lesson-title"><span class="lesson-no">${no}</span>${esc(l.title)} <span class="mins">${l.minutes} នាទី</span></h3><div class="lesson-body">${md(l.content)}</div>`;
    if (l.quiz) {
      ...
      l.quiz.questions.forEach((q, qi) => {
        ...
        else if (q.type === 'numeric') body += `<div class="ans">ចម្លើយ៖ <b>${esc(q.correctAnswer.value)}</b>${q.correctAnswer.tolerance ? ` (±${esc(q.correctAnswer.tolerance)})` : ''}</div>`;
...
const totalImages = course.modules.reduce((a, m) => a + m.lessons.reduce((b, l) => b + (l.content.match(/!\[[^\]]*\]\(\/content\/biology-grade-12\//g)?.length ?? 0), 0), 0);
...
.lesson-body img{display:block;width:100%;height:auto;margin:16px 0;border:1px solid var(--line);border-radius:14px;background:#fbfaf5}
