...
const SECTION_ICONS = [Cookie, Database, KeyRound, Languages, MonitorCog, ShieldCheck]
const INVENTORY_ICONS = [KeyRound, Languages, MonitorCog, Settings2]

export function CookiesPageContent() {
  const [menuOpen, setMenuOpen] = useState(false)
  const t = useTranslations("cookiesPage")
  const inventory = t.raw("inventory.items") as StorageItem[]
      ...
      <main className="relative z-10 px-5 pb-24 pt-28 sm:px-6 sm:pt-32">
        <section className="mx-auto max-w-7xl">
          <div className="relative overflow-hidden rounded-[2rem] border border-border bg-background/88 px-6 py-10 shadow-[0_28px_80px_-48px_rgba(35,131,226,0.42)] backdrop-blur-sm sm:px-10 sm:py-14 lg:px-14">
            ...
            <div
              aria-hidden
              className="absolute -bottom-24 left-1/3 size-64 rounded-full bg-amber-300/10 blur-3xl"
            />
            ...
            <div className="relative grid items-end gap-10 lg:grid-cols-[minmax(0,1fr)_17rem]">
              ...
              <div className="hidden rounded-[1.75rem] border border-[#2383e2]/16 bg-linear-to-br from-[#2383e2]/10 via-background/85 to-amber-300/10 p-5 lg:block">
        ...
        <section className="mx-auto mt-8 max-w-7xl">
          <div className="rounded-[2rem] border border-border bg-background/92 p-5 shadow-[0_20px_60px_-46px_rgba(31,77,145,0.34)] backdrop-blur-sm sm:p-7">
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              {inventory.map((item, index) => {
                const Icon = INVENTORY_ICONS[index % INVENTORY_ICONS.length]
                return (
                  <div
                    key={item.name}
                    className="landing-interactive-card rounded-[1.35rem] border border-border bg-muted/30 p-4"
                  >
                    ...
                    <div className="mt-4 flex items-center gap-2 border-t border-border pt-3 text-xs text-muted-foreground">
                      <Clock3 className="size-3.5 text-[#2383e2]" />
                      {item.duration}
                    </div>
                  </div>
                )
        ...
        <section className="mx-auto mt-10 grid max-w-7xl items-start gap-8 lg:grid-cols-[17rem_minmax(0,1fr)]">
          <aside className="lg:sticky lg:top-24">
            <div className="rounded-[1.5rem] border border-border bg-background/92 p-4 shadow-[0_18px_50px_-38px_rgba(31,77,145,0.35)] backdrop-blur-sm">
              <div className="mb-3 flex items-center gap-2 px-3 pt-2 text-sm font-semibold">
                <Settings2 className="size-4 text-[#2383e2]" />
                {t("toc")}
              </div>
              <nav
                aria-label={t("toc")}
          ...
          <article className="overflow-hidden rounded-[2rem] border border-border bg-background/95 shadow-[0_24px_70px_-48px_rgba(31,77,145,0.32)]">
            ...
            <div className="m-4 rounded-[1.5rem] bg-linear-to-br from-[#1675d1] to-[#3154bd] p-6 text-white sm:m-6 sm:flex sm:items-center sm:justify-between sm:gap-8 sm:p-8">
              <div>
